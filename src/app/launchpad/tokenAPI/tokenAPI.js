...
const API = {
    ...
    getTokenInfo: tokenAddress => new Promise((resolve, reject) => {
        ...
        .then(token =>
            ...
            .then(name => Promise.all(
                [Promise.resolve(name), token.PRICE.call(), API.getBalance(token.address), token.symbol.call(), token.totalSupply.call(), token.currentPhase.call(), token.tokenManager.call(), token.crowdsaleManager.call()]))
            ...
            .then(info => TokenManager.at(info.tokenManager.address)
                .then(mgr => Promise.all(
                        [API.getBalance(mgr.address), mgr.getOwners.call(), API.getManagerActions(mgr)])
    ...
    setPhase: (tokenAddr, phase, mgrAddr) => {
        ...
        return m.tokenSetPresalePhase(
            tokenAddr, phase, { gas: 500000, from: web3.eth.accounts[0] }
        );
    },
    ...
    withdrawEther: (tokenAddr, mgrAddr) => {
        ...
        return m.tokenWithdrawEther(
            tokenAddr, { gas: 500000, from: web3.eth.accounts[0] }
        );
    },
    ...
    confirmTransaction: (txId, mgrAddr) => {
        ...
        return m.confirmTransaction(
            txId, { gas: 500000, from: web3.eth.accounts[0] }
        );
    },
